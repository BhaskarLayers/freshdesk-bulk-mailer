...
def freshdesk_get(endpoint: str) -> requests.Response:
    url = f"{freshdesk_base_url()}/{endpoint}"
    try:
...
def send_ticket(email: str, subject: str, body: str, custom_fields: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def get_ticket_fields():
    """
    Fetch ticket fields from Freshdesk to get available dispositions.
    """
    resp = freshdesk_get("ticket_fields")
    if not resp.ok:
        raise HTTPException(
            status_code=resp.status_code,
            detail=f"Failed to fetch ticket fields: {resp.text}",
        )
    return resp.json()
...
async def send_bulk(
    ...
    if custom_fields_json:
        try:
            ...
            logger.warning("Invalid JSON provided for custom_fields_json")
            # We proceed without extra fields if JSON is invalid, or we could raise 400.
            # For now, let's proceed to avoid blocking basic functionality.

    # Merge hardcoded disposition with any dynamic extra fields
    final_custom_fields = {
